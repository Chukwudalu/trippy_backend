...
exports.updateMe = catchAsync ( async(req, res, next) => {
    ...
    // Check if the req contains a file
    if(req.file){
        fs.readFile(req.file.path, function(err, data) {
            ...
            imageKit.upload({
              ...
              if(error) return next(new AppError('An Error occured during image upload', 500));
              else{
                    const { url } = response;
                    ...
                    filteredBody = filterObj(req.body, 'name', 'email', 'photo')
                    filteredBody.photo = modifiedUrl;
        ...
        fs.unlink(req.file.path, (err) => {
            if(err) return next(new AppError('An Error occured during image upload', 500));
        })
